...
class App extends React.Component {
    ...
    componentDidMount() {
        ...
        const setPath = this.props.param;

        appLogic.loadFile('sets/' + setPath + '.json').then(() => {
            appLogic.loadSavedState();

            this.setState({
                ...this.state,
                dataName: appLogic.dataName
            });
        ...
        return (
            <div className="app-container h-100">
                ...
                {this.state.showList !== true && this.state.noCards === false &&
                    ...
                    <div className="px-16">
                        <h1 className="page-title">{ this.state.dataName }</h1>

                        <Card word={this.state.cards1.w} trans={this.state.cards1.t} example={this.state.cards1.e} p={this.state.cards1.p}></Card>
                        <Card word={this.state.cards2.w} trans={this.state.cards2.t} example={this.state.cards2.e} p={this.state.cards2.p}></Card>
                        <Card word={this.state.cards3.w} trans={this.state.cards3.t} example={this.state.cards3.e} p={this.state.cards3.p} flipped="true"></Card>
                        <Card word={this.state.cards4.w} trans={this.state.cards4.t} example={this.state.cards4.e} p={this.state.cards4.p} flipped="true"></Card>
                    </div>
                }
                ...
                <footer>
                    <a onClick={() => this.props.openHome() }>
                        <button><Home/></button>
                    </a>
                    <button onClick={() => { this.settings() }}>
                        <SettingsHorizontal/>
                    </button>
                    {!this.state.showList && <button onClick={() => { this.toggleList() }}><ThreeLineHorizontal/></button>}
                    {this.state.showList && <button onClick={() => { this.toggleList() }}><Grid/></button>}
